...
if( $ref=="")
{
	//	$datoparaje="('$id','A','$loc','$sta','$par','$lati','$lon','','$ten','$supe','','$refu','$usu',now(),'$nombre_asociado','$fec','$cam','1','1')";
	$datoparaje="('$id','A','$loc','$sta','$par','$lati','$lon','','$ten','$supe','','$refu','$usu',now(),'','$fec','$cam','1','1')";
}
else
{
     $datoparaje="('$id','A','$loc','$sta','$par','$lati','$lon','','$ten','$supe','','$refu','$usu',NOW(),'$ref','$fec','$cam','1','1')";
}
...
$sqlparaje="INSERT INTO paraje (id_paraje,distincion,id_localidad,id_cliente,paraje,lat,lng,poligono,tenencia,superficie,docpro,referencia,usufruto,fecha,nombrep,fecha_paraje,rcampo,status,tipo)VALUES".$datoparaje;
...
$datoconst="('A',now(),'constancia$id_parajito' ,'$id_parajito','1')";
		 
		//  $count_id++;	
	  //}
	  $sqlconstancia="insert into constancias(distincion,fecha,constancia,id_paraje,status)values".$datoconst;
...
//{
	...
	if ($value[5]>4 && $sin==1) {
	 ...
	 // $datoextracc="('$id_parajito','1',now(),' '),('$id_parajito','1',now(),' '),('$id_parajito','1',now(),' '),('$id_parajito','1',now(),' '),('$id_parajito','1',now(),' ')";
	  $datoextracc="('$id_parajito','A','1',now(),' ','$sin'),('$id_parajito','A','1',now(),' ','$sin'),('$id_parajito','A','1',now(),' ','$sin'),('$id_parajito','A','1',now(),' ','$sin'),('$id_parajito','A','1',now(),' ','$sin')";
	 
	}
	
	
	else if ($value[5]>4 && $sin==2){
    $datoextracc="('$id_parajito','A','1',now(),' ','$sin')";
