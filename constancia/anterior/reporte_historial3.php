...
function Footer()
{
	...
	$strConsulta = "SELECT Date_format(constancias.fecha,'%y') as anio,
character_length(CONCAT(calle, ' #', noexterior,', ',colonia,', ',municipios.nombre,', ',estados.nombre)) as contador,
CONCAT(calle, ' #', noexterior,', ',colonia,', ',municipios.nombre,', ',estados.nombre) as domicilio,nombrep,clientes.no_cliente,regmaguey,LPAD(constancias.id_constancia,4,'0') as constancia,CONCAT(LPAD(paraje.id_paraje,4,'0'),paraje.distincion) as parajes,clientes.nombre as clienten,clientes.calle,clientes.noexterior,clientes.nointerior,clientes.colonia, municipios.nombre as nombrem,estados.nombre as nombree,clientes.telefono,clientes.correo,constancias.fecha as fecha1,date_add(constancias.fecha, INTERVAL 1 YEAR) as fecha2
...
$fecha2 = ucfirst($fechaa);
	...
	$pdf->Ln(10);
	$pdf->Cell(0,3, utf8_decode(strtoupper('N° DE REGISTRO CRM: ')).$fila['no_cliente'],0,5, 'C');
	...
	$pdf->SetFont('Helvetica','B',10);
	 ...
	 $pdf->Cell(56,9,''.'    '.'',1,0,'C');
	 $pdf->SetX(140);
	 $pdf->Cell(56,7,utf8_decode($dato['lat']).'        '.utf8_decode($dato['lng']),0,0,'C');
	 $pdf->Ln(0);
	 $pdf->SetX(129);
	 $pdf->SetFont('Helvetica','B',7);
...
// Aqui empieza la tabla de atributos de la tierra
	...
	$pdf->SetFont('Helvetica','B',15);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(0,12, strtoupper('TIPO DE MAGUEY'), 0,5, 'C');
	$pdf->Ln(0);
...
$pdf->SetFillColor(85,107,47);
	...
	$pdf->SetFont('Helvetica','B',8);
	$pdf->Cell(94,5,utf8_decode('TIPO DE MAGUEY'),1,0,'C',1);
	...
	$strConsulta = "SELECT  existenciaplanta.regmaguey, existenciaplanta.cantidadini, existenciaplanta.edad, comun.nombre,especie.genespecie,especie.variante
...
where paraje.id_paraje='$paraje'";	
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			$pdf->Cell( 88, 20, $pdf->Image("images/firmae.jpg", $pdf->GetX(), $pdf->GetY(),40.78), 0,0, 'C', false );
			$pdf->Ln(18);
			$pdf->cell(180,5,utf8_decode('M. EN C. EFRAÍN PAREDES HERNÁNDEZ'),0,0,'C');
			//$pdf->cell(88,5,utf8_decode('DR. EN C. HIPÓCRATES NOLASCO CANCINO'),0,0,'C');
			$pdf->Ln(0);
			$pdf->cell(180,5,utf8_decode('_______________________________________'),0,0,'C');
			//$pdf->cell(88,5,utf8_decode('_______________________________________'),0,0,'C');
			$pdf->Ln(5);
			$pdf->SetFont('Helvetica','B',9);
			$pdf->cell(180,5,utf8_decode(strtoupper('COORDINADOR DE LA UNIDAD DE MAGUEY')),0,0,'C');
	...
	//  aqui empieza el pdf de ec¿xtraccion
	
	
	
	
	
	// aqui termina extraccion

$pdf->AliasNbPages();	
			ob_end_clean();
   $pdf->Output("Registro".$id.".pdf",'D');
