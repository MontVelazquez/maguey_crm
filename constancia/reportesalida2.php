...
regmaguey,LPAD(constancias.id_constancia,4,'0') as constanciae,CONCAT(LPAD(paraje.id_paraje,5,'0'),paraje.distincion) as parajes,
...
where paraje.id_paraje='$paraje'";
	...
	$strConsulta = "SELECT CONCAT(cextracciones.id_extraccion,cextracciones.distincion) as id FROM cextracciones WHERE id_paraje = '$paraje' ORDER BY id_extraccion";
...
$fecha2 = ucfirst($fechaa);
	...
	$pdf->SetFont('Helvetica','B',10);
	//$pdf->Text(189,33,$year,0,5,'C');
	$pdf->Text(189,33,"INDEFINIDA",0,5,'C');
	$pdf->Ln(14);
...
FROM existenciaplanta Inner Join comun ON comun.id_comun= existenciaplanta.id_comun Inner Join especie ON comun.id_especie = especie.id_especie 
Inner Join paraje ON paraje.id_paraje=existenciaplanta.id_paraje
	WHERE  paraje.id_paraje='$paraje' and existenciaplanta.edad > 4 AND existenciaplanta.existenciaplantas>0";	
	...
	$pdf->Cell(68,7,'No. DE REGISTRO COMERCAN: ',0,0);
	$pdf->SetFont('Helvetica','B',10);
