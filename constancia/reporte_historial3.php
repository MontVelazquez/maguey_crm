...
CONCAT(calle, ' #', noexterior,', ',colonia,', ',municipios.nombre,', ',estados.nombre) as domicilio,nombrep,clientes.no_cliente,regmaguey,LPAD(constancias.id_constancia,4,'0') as constancia,CONCAT(LPAD(paraje.id_paraje,5,'0'),paraje.distincion) as parajes,clientes.nombre as clienten,clientes.calle,clientes.noexterior,clientes.nointerior,clientes.colonia, municipios.nombre as nombrem,estados.nombre as nombree,clientes.telefono,clientes.correo,constancias.fecha as fecha1,date_add(constancias.fecha, INTERVAL 1 YEAR) as fecha2
...
$fecha2 = ucfirst($fechaa);
	...
	$pdf->Ln(10);
	$pdf->Cell(0,3, utf8_decode(strtoupper('N° DE REGISTRO COMERCAM: ')).$fila['no_cliente'],0,5, 'C');
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			$pdf->Ln(5);
			$pdf->SetFont('Helvetica','B',9);
			$pdf->cell(180,5,utf8_decode(strtoupper('JEFE DE DIVISIÓN AGRÍCOLA')),0,0,'C');
