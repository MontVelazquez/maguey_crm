...
function Footer()
{
	...
	$paraje= $_GET['id'];
	$strConsulta = "SELECT Date_format(cextracciones.fecha,'%y') as anio,nombrep,paraje.tenencia,paraje.id_paraje,clientes.no_cliente,regmaguey,LPAD(cextracciones.id_extraccion,4,'0') as constanciae,LPAD(paraje.id_paraje,4,'0') as parajes,clientes.nombre as clienten,clientes.calle,clientes.noexterior,clientes.nointerior,clientes.colonia, municipios.nombre as nombrem,estados.nombre as nombree,clientes.telefono,clientes.correo,cextracciones.fecha as fecha1,date_add(cextracciones.fecha, INTERVAL 1 YEAR) as fecha2 
from estados 
inner join municipios on municipios.estado=estados.clave
inner join clientes on clientes.municipio=municipios.id 
inner join paraje on paraje.id_cliente=clientes.no_cliente
inner join cextracciones on cextracciones.id_paraje=paraje.id_paraje 
...
where paraje.id_paraje='$paraje'";
	...
	$strConsulta = "SELECT LPAD(cextracciones.id_extraccion,4,'0') as id FROM cextracciones WHERE id_paraje = '$paraje' ORDER BY id_extraccion";
...
$fecha2 = ucfirst($fechaa);
	...
	$pdf->Text(170,27,strtoupper($extraccion['id']).'/'.strtoupper($fila['parajes']),0,5,'C');
	$pdf->SetTextColor(33,33,33);
	$pdf->SetFont('Helvetica','B',10);
	$pdf->Text(170,33,utf8_decode('VIGENCIA: '),0,5,'C');
	$pdf->SetFont('Helvetica','B',10);
	$pdf->Text(189,33,$fecha2,0,5,'C');
	$pdf->Ln(14);
