...
function Footer()
{
	...
	//$paraje= $_GET['id'];
	$strConsulta = "SELECT Date_format(constancias.fecha,'%y') as anio,nombrep,tenencia,paraje.id_paraje,clientes.no_cliente,
regmaguey,LPAD(constancias.id_constancia,4,'0') as constanciae,LPAD(paraje.id_paraje,4,'0') as parajes,
...
LIMIT 1";
	...
	//$strConsulta = "SELECT LPAD(cextracciones.id_extraccion,4,'0') as id, MAX(paraje.id_paraje) as id_paraje FROM cextracciones inner join paraje on cextracciones.id_paraje=paraje.id_paraje ORDER BY id_extraccion";
	$strConsulta = "SELECT LPAD(cextracciones.id_extraccion,4,'0') as id FROM cextracciones WHERE id_paraje = ( SELECT MAX(paraje.id_paraje) AS parajito FROM paraje) ORDER BY id_extraccion";
...
$fecha2 = ucfirst($fechaa);
	...
	$pdf->Text(183,27,strtoupper($extraccion['id']).'/'.strtoupper($fila['parajes']),0,5,'C');
	$pdf->SetTextColor(33,33,33);
	$pdf->SetFont('Helvetica','B',10);
	$pdf->Text(180,33,utf8_decode('VIGENCIA: '),0,5,'C');
	$pdf->SetFont('Helvetica','B',10);
	$pdf->Text(199,33,$fecha2,0,5,'C');
	$pdf->Ln(14);
	...
	$pdf->SetFont('Helvetica','B',9);
	$pdf->Cell(152,7,utf8_decode($fila['nombrep']),0,5,'C');
	$pdf->Ln(-7);
	$pdf->SetFont('Helvetica','',10);
	...
	$Consulta = "SELECT localidades.localidad,municipios.nombre as nombrem,estados.nombre as 		nombree,paraje.paraje,paraje.referencia,paraje.lat,paraje.lng,paraje.superficie
		...
		inner join localidades on localidades.MunicipioID=municipios.id 
		inner join paraje on paraje.id_localidad=localidades.id 
 ORDER BY id_paraje DESC LIMIT 1
		";
...
Inner Join paraje ON paraje.id_paraje=existenciaplanta.id_paraje
 WHERE existenciaplanta.edad > 4 and paraje.id_paraje=(
  SELECT MAX(paraje.id_paraje) AS parajito
  FROM  paraje)
";	
	...
	$pdf->Rect(10, 39, 195, 88 + ($numfilas - 1) * 5); /*es el cuarto numero*/
	...
	$pdf->SetFillColor(255,255,255);
	$pdf->Rect(10, 132 + ($numfilas - 1) * 5, 195, 65+ ($numfilas - 1) * 5);
				}
	...
	$pdf->Ln(1);
	$pdf->SetFont('Helvetica','B',10);
	$pdf->Cell(34,7,'No. DE ASOCIADO: ',0,0);
	$pdf->SetFont('Helvetica','',9);
	$pdf->Cell(33,7,'___________________',0,0,'C');
	$pdf->SetFont('Helvetica','B',10);
	...
	$Consulta = "SELECT localidades.localidad,municipios.nombre as nombrem,estados.nombre as 		nombree,paraje.paraje,paraje.referencia,paraje.lat,paraje.lng,paraje.superficie
		...
		inner join municipios on municipios.estado=estados.clave 
		inner join localidades on localidades.MunicipioID=municipios.id 
		inner join paraje on paraje.id_localidad=localidades.id ORDER BY paraje.id_paraje DESC LIMIT 1";
...
Inner Join paraje ON paraje.id_paraje=existenciaplanta.id_paraje
 WHERE existenciaplanta.edad > 4 and paraje.id_paraje=(
  ...
  FROM  paraje)";	
    ...
    $Consultapdf = "SELECT * from existenciaplanta inner join paraje on existenciaplanta.id_paraje=paraje.id_paraje
 WHERE existenciaplanta.edad > 4 and paraje.id_paraje=(
  SELECT MAX(paraje.id_paraje) AS parajito
  FROM  paraje)";	
	$salepdf= $conexion->query($Consultapdf);
	$numerito = mysqli_num_rows($salepdf);
    
	if ($numerito==1){				
				
   ob_end_clean();
   $pdf->Output("Extraccion.pdf",'D');
	}else{
	   }
